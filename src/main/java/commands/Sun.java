...
public class Sun implements CommandExecutor {
    ...
    public Sun(DynamicAtmosphere plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player || sender instanceof ConsoleCommandSender) {
            // Verificar permisos
            if (sender.hasPermission("dynamicAt.sun") || sender instanceof ConsoleCommandSender || sender.isOp()) {
                // Obtener el mundo actual
                World world = ((Player) sender).getWorld();

                // Desactivar la lluvia y la tormenta
                world.setStorm(false);
                world.setThundering(false);

                // Obtener el mensaje de tiempo desde la configuración
                ...
                timeSetMessage = timeSetMessage.replace("%dy-time-en%", "Sunny");
                timeSetMessage = timeSetMessage.replace("%dy-time-es%", "Soleado");
